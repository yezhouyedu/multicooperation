...
> 历史供应商参考：本文针对 WordPress 5.7.1 和宿主机 Nginx，不适用于本项目的 Docker Compose + Nginx 架构。项目实际证书目录、续期、回滚和验证命令以同目录 `HTTPS域名接入方案.md` 为唯一执行依据。不要直接复制本文的路径、systemd 命令或旧 TLS 配置到生产环境。
...
2. 参见 [使用OrcaTerm登录Linux实例](https://cloud.tencent.com/document/product/1207/44642) 登录轻量应用服务器，或参见 [远程登录Linux实例](https://cloud.tencent.com/document/product/1207/44578) 在本地计算机中使用远程登录工具（如 WinSCP），通过用户名密码方式或者 SSH 密钥对方式登录轻量应用服务器。
...
>   - 如您找不到上述目录，可执行命令 nginx -t ，找到nginx的配置文件路径。

4. 对于 WordPress 镜像创建的实例则执行以下命令，编辑 Nginx 默认配置文件目录中的 `nginx.conf` 文件。
...
执行以下命令，验证配置文件是否存在问题。
   ...
   sudo nginx -t
   ```
  - 若输出信息如下图所示，则为配置成功，请继续执行 [步骤8](https://cloud.tencent.com/document/product//#Step8)。
![](https://qcloudimg.tencent-cloud.cn/image/document/9e97db0b00d6d8659ab2dbe463f70a14.png)
...
您需要对 `nginx.conf` 文件进行修改，在“证书安装”中的 [步骤4](https://cloud.tencent.com/document/product//#Step4) 的配置之后继续添加如下配置：
   ...
   ```
2. 保存修改后的 `nginx.conf` 文件后退出，参考“证书安装”的 [步骤7](https://cloud.tencent.com/document/product/1207/47027#Step7) 及 [步骤8](https://cloud.tencent.com/document/product//#Step8) 验证并重启 Nginx。
至此已成功设置 HTTPS 的自动跳转，您可使用 `http://cloud.tencent.com`（示例）自动跳转至 HTTPS 页面。如下图所示：
![](https://qcloudimg.tencent-cloud.cn/image/document/4f6e666518b31adca6f687c72b98edc0.png)
