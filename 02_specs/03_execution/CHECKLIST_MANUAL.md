...
- 高级 AI 图片选择器失焦标记 `authorizedDialog=true`；在 Session 快照的授权上限内不判违规，超过默认 60 秒上限后按普通切屏阈值判定。
...
- 指导语页能进入
- A/B 第一页均显示固定报酬 20 元、线上实验参与规则和规则确认；第五节标题后显示“为保证线上实验按照统一方式进行，请在进入后续实验前阅读以下规则。”
- A 指导语将完全无相关线索的材料统一表述为选择“未发现”；A 任务表的团队绩效口径为“A和B均经手处理并有提交记录的公司”
...
- 三个段前页按 V0.4 显示：中性组 N1/N2/N3、合作组 C/V/S，团队内各出现一次且顺序随机；双方完成后才启动工作段。
- 三次工作段回顾和最终问卷使用 V3.0；导出必须保存 `displayedItemCodes`、`displayContext` 与原始答案。
- POST-INT-01/02 对所有完成者显示；只有 POST-INT-01 回答“有”时显示并保存 POST-INT-03。
- 在 Admin 手动修改合法问卷文案后重启 server，确认初始化逻辑不会恢复旧文案。
- 合作叙事条件每段抽取当前主题 20 题，5 个 `content_subtype` 各 4 题；中性条件不得误用合作题。
- 正式题库版本核对为 V1.5.1，active 共 900 题。
...
- 材料区页面内截图可生成预览；ADVANCED 可发送截图，BASIC 和 A0/A7/A8 不得发送图片。
