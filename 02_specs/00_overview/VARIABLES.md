...
> 状态：2026-09-06 九条件、V0.4、V3.0 与线上实验质量口径版
...
### 3.3A 段前指导语
- runtime 阶段 `pre_segment_instruction`
- `instructionOrderSeed`、`themeOrderSeed`、工作段对应文本 ID、主题和显示顺序
- 每名参与者的打开、开始、完成时间和阅读时长
- 中性组 N1/N2/N3、合作组 C/V/S 在团队内各出现一次
...
- 剪贴板只保存字符数、带 Session 盐的哈希、内外来源分类、是否紧邻切屏，以及当时的工作段/任务/公司关联，不保存原文；当前标签页刷新后仍保留最近 50 个平台内复制哈希用于分类
...
- 副线已完成累计只在当前工作段内有效
- 截图按钮属于页面内材料处理工具；图片发送 AI 时沿用附件事件、`imageRef` 和相对路径，不采集整屏原始截图日志
