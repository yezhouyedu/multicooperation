...
- 先做测试题，再进入测试轮工作台
- 测试轮工作台内要完成主线与副线教学引导
- 测试轮真实副线任务数固定为 0；后端不为 `segmentIndex = 0` 创建或返回真实 `SideTaskPlan`
- 副线教学只在前端副线面板内临时注入一个 `practice_demo_sidetask` 演示题；点击只推进教学步骤，不调用正式副线作答接口，不进入副线成绩变量
...
测试轮 `segmentIndex = 0` 不生成真实副线计划，也不会按 30 秒节奏释放后台副线题；教学期看到的副线演示题是前端临时题，不属于 `SideTaskPlan`。
...
测试轮教学期间，如果真实副线队列为空，前端会在副线展开页内显示一个教学演示题，帮助被试完成“点击副线答案”的教学步骤；此时真实待处理计数仍为 `0 / 0`，教学完成后不再显示该演示题。
