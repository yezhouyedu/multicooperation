...
- 开篇页固定按“通用指导语 → 任务 1：公司信息处理 → 任务 2 与报酬 → 角色说明 → 知情同意说明”呈现。前三章严格遵循《通用指导语 7.5改》的原文顺序；A0-A6 的 AI、提醒和叙事处理由实验条件快照决定，不在开篇页展示条件名称。
- 工作台通过“无 AI / aiseek / aiseek pro”的实际界面状态表达 AI 条件，参与者前台不显示 A0-A6 编号或处理条件名。
...
提醒与叙事不再各自独立随机；Session 领取 A0-A6 条件后按固定映射得到 `continuous/batch` 与 `neutral_info/coop_narrative`。同一 Session 内 A/B 共享该处理，整个实验期间不变。
