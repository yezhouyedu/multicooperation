...
- 开篇指导语分两页：第 1 页阅读通用指导语和当前 A/B 角色指导语，第 2 页只读预览当前角色任务表
- 任务表预览第 2 页确认后，才进入测试题前同步准备页，再进入测试题
...
- B 发送反馈时，`b_feedback_to_a` 事件关联 B 来源任务/公司，并由服务端快照 A 此刻仍在处理的任务/公司；不另建 A 收到回执。
- B 解锁后打开 A 原始材料时，以成对事件记录页面可见且获焦的区间；A 材料之间切换不中断，失焦、换到其他标签、换公司或离开工作段时结束。
...
- `POST /admin/companies/library/replace-upload`
  - 浏览器文件夹上传替换材料库；支持“替换选中公司”和“全部替换”，导入前校验目录结构，替换前备份旧材料文件。
