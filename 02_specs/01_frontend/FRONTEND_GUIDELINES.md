...
## 1. 总目标
- 让 A/B 在同一套稳定工作台内完成测试轮、正式实验、任务2处理、工作段回顾与休息等待切换
...
- 测试轮复用正式工作台骨架
- 工作段回顾/休息页保持统一顶栏，但主体切换为回顾问卷或等待内容

## 3. 顶栏规则
- 显示平台名、角色、当前项目/公司、保存状态、当前阶段倒计时
- 主工作台、测试轮页、工作段回顾/休息页、任务2展开页统一显示当前阶段倒计时
...
- B 材料区必须有 `A信息` tab；A 反馈未解锁前显示锁定态，解锁后显示 A 的任务1反馈
- B 作答区只放 B 自己的投资判断表，不把 A 的反馈卡片塞进作答区
- B 提交按钮在 A 反馈未解锁前保持灰态，并用 hover/title 说明原因
- `A信息` 是否被点击只做行为记录，不再作为提交门槛

## 5. 任务2区规范
- 顶部入口固定文案：`任务2`
- 不显示数字
- 不显示“市场实时快讯”
- 滚动文案固定：`任务2有新题目，请及时处理`
- 滚动方向固定为从右往左
- 文案到达最左端后停留 `5s`
- 停留结束后用 `2s` 左右淡化消失
- 消失后间隔 `30s` 再次从右往左进入
...
## 8. 材料与表单规范
- 材料区已从早期 `P01 + 两份正式表单` 口径扩展到正式材料系统，当前以真实案例库与角色分层材料为准
- `.txt` 直接显示
- `.docx` 以可读优先的结构化阅读器展示
