...
本目录存放项目规格文档，按“总览 / 前端 / 后端 / 执行验收 / 上线前数据 / 服务器上线 / 韧性研究”分层。
...
- `06_resilience/`
  - 放断网、刷新、服务故障等韧性研究与实现记录；当前流程规则仍以 `APP_FLOW.md` 和执行 specs 为准
...
- `HTTPS部署/Nginx 服务器证书安装.md`：供应商通用参考，不作为本项目部署命令

### 06_resilience
- 日期命名的研究与实施记录；仅在处理韧性任务时按需读取
