...
**背景**：`aiseek.tech` 公安备案通过，需要在网页源码底部展示公安备案图标和备案号链接。

**本轮实现**：
- 新增 web 静态资源 `apps/web/public/beian-police.png`，来源为 `02_specs/05_server_deploy/公安备案/备案图标.png`。
- 在全局 `apps/web/src/app/layout.tsx` 底部加入公安备案 footer。
- 备案链接为 `https://beian.mps.gov.cn/#/query/webSearch?code=37098202000886`，显示文案为 `鲁公网安备37098202000886号`。
- 样式在 `globals.css` 中固定到页面右下角，尽量避免影响实验主流程页面布局。
...
### 2026-06-18 休息问卷段 UI 美化

**背景**：用户反馈休息问卷段的 UI 很丑，特别是量表题（1-7）与"非常同意/非常不同意"标签对不齐，整体视觉缺乏精致感。

**本轮实现**：

#### QuestionnaireForm 组件美化（questionnaire-form.tsx）

1. **量表题（scale）重做**：数字按钮改为连续排列的按钮组，选中态蓝色背景+白字+外发光，未选中态白色+灰色边框；标签行两端对齐；新增刻度标记行
2. **单选题/多选题增强**：新增圆形/方形选中指示器（radio/checkbox style），选中时显示 ✓ 图标
3. **输入框优化**：focus 时增加外发光效果
4. **条件追问优化**：增加"追问"标签，渐变背景
5. **进度指示器新增**：顶部显示已完成/总数进度条
6. **提交按钮增强**：增加 shadow、hover 阴影加深、提交中 spinner 动画
7. **提交区域优化**：底部卡片式，左侧完成状态，右侧按钮

#### Break 页面美化（break/page.tsx）

1. **背景优化**：从纯色改为渐变背景
2. **问卷头部重做**：新增图标（📊/📋），标签大写+蓝色，标题加粗
3. **倒计时卡片美化**：增加时钟图标，渐变背景，数字加粗
4. **AI 升级提示美化**：增加 ✨ 图标，渐变背景
5. **提交成功状态美化**：增加绿色圆形 ✓ 图标，渐变背景
6. **加载状态美化**：从静态文字改为 spinner 动画
7. **底部提示新增**："问卷数据已自动保存，请放心作答"
...
- 2026-06-20 追补：优化 `/instruction` 视觉与分组标题，改为“实验流程 / 报酬与任务说明 / 机会与风险怎么判断 / 你接下来要完成的任务”等参与者口吻；admin 指导语积木新增 `experimentFlow`，用于配置开篇页“实验流程”段落。
...
### 2026-06-29 师兄反馈四项修复：AI 体验、B 端 5 分钟门槛、问卷标题

**背景**：师兄测试反馈 AI 偶发慢/失败、AI 回复复制不顺、B 接到新公司后可能立刻查看 A 信息/材料或提交、最终问卷标题出现乱码。本轮按修正版计划做 P0 修复，并同步流程文档。

**实现**：
- AI 等待体验：保留后端现有 `latencyMs`、`ai_wait_started/ended` 记录；前端流式生成超过 10 秒显示“AI 正在生成，可能需要较长时间”，失败时显示中文错误并保留重试/重新生成入口。
...
### 2026-06-29 A 表 v5、任务2滚动与段前/教学文案修复

**背景**：用户提供 `A端尽调表_参与者可见版v5.docx`，要求对齐新版 A.1；同时复查新版公司材料库、修复任务2提示不滚动、调整段前指导语和教学页措辞，并上线服务器。
...
**背景**：测试轮中第一次阅读任务表会影响测试表现，因此在开篇指导语后、测试题前新增一页只读任务表预览，让参与者先熟悉自己后续需要填写的 A/B 任务表。

**实现**：
- `/instruction` 第 1 页按钮从“我已阅读，开始测试题”改为“我已阅读，进行下一步”，不再直接调用测试题 ready。
- 新增 `/instruction/task-preview` 第 2 页：按当前角色展示 A 端任务表或 B 端任务表，只读、可滚动、隐藏滚动条，输入框和选项均禁用，不保存草稿、不写入正式变量。
- 第 2 页底部按钮设置 30 秒阅读倒计时；倒计时结束后才能点击“我已阅读，开始测试题”，随后调用原 `ready-practice` 接口进入同步准备页 1。
- 更新 `APP_FLOW.md`：总流程改为“开篇指导语第 1 页 -> 任务表预览第 2 页 -> 同步准备页 1 -> 测试题”，并补充该页设计目的与变量边界。

**验证**：
- 本地构建通过：`corepack pnpm --filter web build`；Next 构建路由列表已包含 `/instruction/task-preview`。

### 2026-07-01 开场知情同意、B 表修复、AI 复制与 admin 材料替换收口

**背景**：用户要求继续上线前小修：开场指导语补知情同意；修复 B 表普通机会/风险数量自动清空；改善 AI 输出复制；精简 admin 材料管理 tab，并支持浏览器上传文件夹替换材料库；同步若干参与者前台措辞。

**实现**：
- `/instruction` 第 1 页底部新增知情同意说明，位于按钮上方。
- B 端任务区题头改为“B端任务表”；公司信息只保留“公司简称/匿名代号”；重要机会/风险区去掉重复说明；普通机会数和普通风险数加入保存 payload，修复自动保存后清空的问题。
- AI 区复制改为只从消息正文区域读取选中文本，按钮/头像/气泡外文字不进入选区复制；用户正在选中文字时不自动滚到底，降低选区跳动。
- 结束页报酬提醒文案更新；最终问卷按钮改为“提交”；测试轮欢迎卡片补充 AI 和任务2仅正式实验可用等说明。
- admin 材料管理 tab 精简为“公司列表/多选/上传替换/全部替换/侧边栏预览”；后端新增浏览器文件夹上传替换接口，上传后临时还原目录、扫描材料结构，不合规则拒绝，合规则入库并备份旧材料文件到 `storage/material_backups/`。
- 更新 `admin材料库上传手册.md` 为浏览器上传替换口径。

**变量与导出影响**：
- 本轮不新增实验变量、不改变导出目录树、不改变 `side_tasks/`、`timestamps.json`、`variables.json` 的结构；`数据库文件夹手册.md` 当前无需同步修改。
...
### 2026-07-17 A0-A6 指导语、问卷 V2.2 与实验局导出入口收口

**背景**：第八次会议在七条件正式实验逻辑确定后提供了角色 A、角色 B、通用指导语和三章实验问卷 V2.2。旧页面仍有实验 1/2/3、旧职业角色称呼、两次休息问卷及统一展示 AI 题目的残留口径；Session 概览也缺少实验局归属，难以按实验局选中导出。

**参与者文字与流程**：
- 按四份 2026-07-05 至 2026-07-11 Word 稿更新通用指导语、A/B 角色指导语、任务定义、计分和报酬说明。
- 前台及开发预览统一显示角色 A/B、任务1/任务2；A0 等无 AI 条件不再出现要求参与者使用 AI 的提示。
- 测试轮说明改为：测试轮与正式阶段遵循同一实验条件；任务2练习仅为教学演示，不计正式得分和正式变量。
- 正式流程明确为三次段前指导语、三个工作段回顾、两个休息段、最终长问卷、独立付款手机号确认和完成页。

**问卷 V2.2**：
- 正式模板升级为 `three-chapter-questionnaire-v2-2` / `three_chapter_v2_2_20260711`，实现 V2.2 的题号、题干、选项、量表和模块说明。
- 每段固定展示 6 个共通题；仅 A1-A6 且该参与者在对应工作段实际发送过任务1 AI 消息时展示 3 个 AI 体验题。
- 最终问卷按 A/B、A0-A6、实际 AI 使用、图片上传、查看 A 材料、交接备注和反馈行为动态展示；叙事识别选项按 session 稳定打乱，“没有印象”固定在末尾。
- 最终问卷后新增独立付款手机号确认页，只显示掩码手机号并记录确认状态，不把完整手机号写入问卷答案或导出 JSON。
- 旧正式模板会在运行时配置初始化时自动切换到 V2.2；无需 Prisma migration，现有问卷 `items`、提交 `payload`、TaskProgress 和导出 JSON 字段足以保存新结构。

**Session 概览与导出**：
- 每个 Session 显示实验局名称/代码和 A0-A6 条件；手动 Session 明确显示“手动 / 未归属实验局”。
- 新增实验局筛选、“选中当前实验局全部 Session”和筛选范围全选，继续复用既有按 `sessionCodes` 导出接口。
- `variables.json` 升为 schemaVersion 2，问卷摘要新增模板版本、各阶段实际展示题号、展示上下文和付款手机号确认状态；原始问卷文件同时保留展示题目与答案，可区分“条件未展示”与“展示后未作答”。

**文档同步**：
- 更新 `APP_FLOW.md`、`问卷流程方案.md`、`变量记录与服务器导出方案.md`、`数据库文件夹手册.md`，以 A0-A6、V2.2 三次工作回顾和独立付款确认为当前口径。
- 历史数据库枚举和快照字段继续保留以读取旧 session，但不再作为新正式 session 的 UI 或分配入口。

**验证**：
- server 全量单元测试 3 个测试套件、14 项全部通过；新增问卷条件分支 4 项，覆盖 A0 隐藏 AI、工作段索引、B 角色 A0 文案和高级 AI 图片/行为条件。
...
### 2026-07-17 Admin 问卷页面流转与组装规则说明

**实现**：
- 在 Admin“问卷配置”顶部新增页面流转表，明确平台外招募问卷、`/practice-quiz`、`/break`、`/workspace/end` 第三段回顾、最终长问卷和独立支付确认的内容来源与提交去向。
- 明确本页编辑器保存的是 V2.2 全部可能题目；参与者实际题目由 server 按 A0-A6、A/B 角色和后台行为动态过滤。
...
### 2026-07-17 通用指导语 7.5 页面顺序重建

**背景**：开篇页原先把可配置 `experimentFlow` 作为第一章，并将《通用指导语 7.5改》中的任务1计分、任务2和报酬提前放入“任务介绍”，再把机会/风险定义单独移到第三章，导致内容和顺序与最新版 Word 不一致。

**实现**：
- 删除参与者开篇页的可配置“实验流程”和旧“本轮额外提示”，同时移除 Admin 中已不再生效的实验流程编辑框；底层历史字段继续保留用于兼容旧快照。
- 固定五章顺序：通用指导语、任务 1：公司信息处理、任务 2 与报酬、角色说明、知情同意说明。
- 前三章严格按《通用指导语 7.5改》顺序重建，只统一“任务 1/任务 2、角色 A/角色 B”等项目排版；角色说明和知情同意内容不变。
- 通用指导语使用四阶段流程线；任务1使用定义表、A/B职责对照和计分层级；任务2使用计分行、报酬公式和测试轮提醒。所有视觉摘要旁均保留 Word 完整正文。

**验证与上线**：`apps/web` production build 通过，共 24 个路由。提交 `3cb3092 按最新版重排通用指导语` 已推送 GitHub `main` 并部署 production web；线上 `/instruction` 与 `/api/health` 均返回 200。
...
### 2026-07-18 实验局暂停续领、切换与受保护删除

**背景**：正式实验中途插入测试实验局后，旧实验局会进入 `CLOSED` 且无法再次激活，导致不能回到原序列继续分配。实验局还缺少删除入口和关联 Session 保护。

**实现**：
- 将数据库状态 `CLOSED` 的当前含义统一为“已暂停”：Admin 可重新激活旧实验局，切换时原活动实验局自动暂停，目标实验局继续领取数据库中全局位置最小的可用槽位，不重新生成 seed、区组或槽位，不重置累计分配。
- 切换到手动 / 通用模式时同步暂停当前正式实验局，避免配置已进入手动模式但实验局仍显示 `ACTIVE`。
- 新增实验局删除接口与双层保护：活动实验局禁止删除；只要 `Session.experimentRunId` 仍有关联记录就拒绝删除并提示先在 Session 概览清理对应被试数据。清理后可删除实验局及其槽位。
- Admin 显示“未启动 / 正在分配 / 已暂停”、关联 Session 数、“继续此实验局 / 暂停实验局 / 删除”操作；切换和删除均有确认提示，后端错误正文会直接展示。
- 当前活动实验局继续以 `ExperimentConfig.activeExperimentRunId` 为唯一数据库指针，Session 概览、筛选和导出继续读取持久化的 `experimentRunId` 与 `experimentCondition`，不依赖浏览器缓存。
...
### 2026-08-10 Markdown 当前口径冲突收口

**范围**：本轮只修改 Markdown，不改业务代码、数据库或部署状态。

**已统一**：
- 根 README、项目规则、规格导航、前后端 README 与部署手册统一以 A0-A6 七条件、正式实验局和平衡区组原子领取为当前入口；旧实验 1/2/3 明确降级为历史 Session 兼容资料。
- `APP_FLOW.md`、`PRD.md`、任务2规格与历史执行计划统一当前完整流程：两页开篇指导语、两次同步准备、测试题、测试轮、三次段前指导语、三个工作段、三次工作段回顾、最终长问卷与独立支付确认。
- B 端规则统一为每次分配后拥有自己的 5 分钟窗口；只有 `A 已提交 && bCanSubmitAt <= now` 才能查看 A 信息、解锁 A 原始材料并提交。
- 任务2提醒方式与叙事组别改为由 A0-A6 固定映射，不再独立随机；题目到达间隔改为读取 Session 创建时固化的后台配置，不再在规格中写死 30 秒。
- 变量持久化、服务器导出、自检表和数据库文件夹手册统一当前 `ExperimentRun / ExperimentConditionSlot / Session` 快照、三段回顾与最终问卷文件结构。
- 当前部署文档统一 postgres/server/web/nginx 四服务、HTTPS、正式 P01-P36、测试 P37、数据库卷与 server storage 卷口径。

**审计发现但未改代码**：
- `TaskAssignment.bCanSubmitAt` 已由运行时使用，但当前导出元数据未显式输出该字段；文档已标记为审计风险，现阶段可按 `bAssignedAt + 300 秒` 推导。
- server 生成的导出自检说明仍残留 `break_1 / break_2` 与“后测问卷”旧文案；因本轮范围仅为 Markdown，留待代码修订任务处理。

**验证**：
- `git diff --check` 通过。
- 本轮涉及的 Markdown 均通过严格 UTF-8 解码，未发现 U+FFFD 或连续问号乱码。
- 本轮修改文件中的相对 Markdown 链接均能解析到现有路径。
- 未修改或清理用户已有的 `.obsidian/`、`.playwright-cli/`、部署图片及 nginx 未跟踪目录。
...
### 2026-08-31 HTTPS 证书续期与生产替换

**背景与结论**：
- 腾讯云控制台显示旧证书即将于 2026-09-15 到期；只在控制台申请并签发新证书不会自动替换自建 Docker Nginx 中的现用证书。
- 部署前从公网确认 `aiseek.tech` 实际仍返回旧证书，序列号 `08CED164C274024368AE2CF7FA781A09`，到期时间为 2026-09-15 07:59:59（北京时间）。
- 使用腾讯云下载的 Nginx 证书包完成续期；新证书序列号为 `0C209191A5272082A38AA812B8EF0F34`，有效期至 2026-11-29 07:59:59（北京时间），SAN 同时覆盖 `aiseek.tech` 与 `www.aiseek.tech`。

**安全校验与部署**：
- 部署前分别在本机与服务器计算证书公钥和私钥导出公钥的 SHA-256，结果一致；证书与私钥配对确认通过。
- 旧证书和私钥已备份至服务器 `/etc/multi-cooperation/cert-backups/20260831T164017+0800`。
- 新证书已替换 `/etc/multi-cooperation/certs/aiseek.tech_bundle.crt` 与 `/etc/multi-cooperation/certs/aiseek.tech.key`，`nginx -t` 通过后执行平滑 reload；证书文件、私钥和下载压缩包均未加入 Git。

**生产验收**：
- 独立公网 TLS 握手已实际返回新证书，序列号、域名覆盖和 2026-11-29 到期时间与下载证书一致，系统标准证书链校验通过。
- `https://aiseek.tech/api/health` 返回 200 和 `status=ok`；生产 Nginx 配置检查通过。
- 服务器旧证书备份存在，上传中转目录已删除；本次没有修改数据库、实验局、业务代码或容器镜像。
...
### 2026-09-06 第十次会议实验文本、问卷 V3.0、段前 V0.4 与任务2题库收口

**文本来源与页面修改**：
- 逐项读取并对照第十次会议的角色 A/B 第一页指导语、A端任务表 v6.1、B端任务表 v4、三章实验问卷 V3.0、段前指导语 V0.4 和 `任务2题库20260906.xlsx`；Word 仅作来源，正式默认文本写入代码/数据库模板，Admin 的合法人工编辑能力保留。
- `/instruction` 补齐任务1的 A→B 异步信息链、团队绩效定义、线上实验参与规则和规则确认/知情同意；固定报酬由旧页面 25 元更正为终版 20 元。
- A表绩效口径改为“经手处理并有提交记录的公司”，A.1 按指标名与记录同时正确计分；B表统一使用“证据来源”，并同步更新个人/团队绩效说明。页面字段结构和保存 JSON 不变。
- 未改退出业务规则：继续以项目冻结口径为准，B 退出后 A 可继续；A 退出后 B 停止。问卷 Word 中的背景描述不用于覆盖这条运行时规则。

**问卷 V3.0 与 Admin 防回滚**：
- 正式模板升级为 `three-chapter-questionnaire-v3-0` / `three_chapter_v3_0_20260816`。新的模板 ID 使生产从 V2.2 一次性切换到 V3.0；切换完成后，只要模板结构有效，启动和读取配置都不会反复覆盖 Admin 后续人工修改。
- 更新合作模块说明与 `POST-AICHG-01..04` 统一反事实措辞；旧 `POST-ONLINE-01..04` 替换为 `POST-INT-01/02/03`。线上实施模块位于人口统计之后、报酬清晰度之前，`POST-INT-03` 仅在 `POST-INT-01=有` 时显示。
- 前端按 `showIf` 动态计算题目、必填数和进度；提交前清除隐藏题残留答案，服务端再次计算实际显示题并只保存对应的 `displayedItemCodes/items/answers`。`finalSelfReport` 保存新的 POST-INT 原始答案，同时兼容历史 POST-ONLINE 数据。

**V0.4 新增正式随机约束**：
- 六段段前材料替换为技术中性的 V0.4 候选 B 文本，文本 ID 和快照版本升级为 `_V2` / `pre_segment_instruction_v2`。
- 正式中性条件不再固定 N1→N2→N3，而是每个团队从六种排列中随机一种；三篇各出现一次，A/B 共用，`instructionOrderSeed`、顺序和文本 ID 在 Session 初始化时固化。此规则用于拆开文本差异与工作段熟练/疲劳位置效应。
- 合作条件的 C/V/S 仍各出现一次并随机排序；同时修复旧中文主题映射乱码可能导致 C/V/S 回退到同一文本的问题。
- 合作组每段 20 道当前主题题不再无分层抽取，改为该主题 5 个 `content_subtype` 各 seeded 抽 4 题，再与 20 道普通中性题合并洗牌。这是 V0.4 新增的实际暴露硬约束，用于避免不同团队因 subtype 偶然偏科而接受不同内容浓度。

**任务2 V1.5.1 题库**：
- 新工作簿结构验收通过：`题库` + `结论及修改清单` 两张表，17 列、900 个唯一题号；三段各 300，普通中性池 360、合作叙事池 540，A/B 正确选项各 450，正文必填完整且合作 subtype 容量满足每格至少抽 4 题。
- Admin 导入兼容 `题库` / `正式题库`、中文说明换行英文键两类表头；把 `content_subtype/question_type/business_scenario/text_form` 等新字段映射到既有数据库字段，并从修改清单识别 V1.5.1。
- 导入改为“先全量校验、再事务 upsert、最后停用不在新文件中的旧题”；坏文件不会先破坏线上活动题库。真实文件本地无数据库预演结果为 900/900 通过；本机 Docker Desktop 当时未启动，因此没有改本机运行数据库。

**文档、测试与生产**：
- 更新 `APP_FLOW.md`、`PRD.md`、`段前指导语方案.md`、`问卷流程方案.md`、任务2 spec、变量持久化/导出方案、自检表、线上质量方案和数据库文件夹手册；明确两项随机规则是 V0.4 新增约束及其设计目的、字段映射、seed、快照和导出口径。
- server 8 个测试套件/36 项测试通过；server build、web production build（24 路由）、`git diff --check` 和 UTF-8 乱码检查通过。新增测试覆盖中性随机顺序、中文主题映射、5×4 subtype 抽样、V3.0 顺序/POST-INT 条件题和双语 Excel 字段映射。
- 主实现提交 `e6816d3 精修实验文本并落实V0.4随机规则` 已推送 GitHub `main`，并以 git archive 全量重建生产 web/server。
- 部署前 PostgreSQL 备份：`/opt/multi-cooperation-backups/pre_text_v04_20260906123608.dump`，313310 字节，SHA-256 `d1093ad924286a1304323d0c0d40ad5356cbe69dde027d924a83d3d2451f8aac`。
- 生产活动问卷已核对为 `three-chapter-questionnaire-v3-0` / V3.0；通过受保护 Admin 导入 V1.5.1：900 条全部更新、0 新建、0 停用，当前三段各 300、普通中性 360、合作叙事 540、`directAiCount=0`，数据库版本均为 V1.5.1。
- 公网 `/api/health`、`/login`、`/admin` 均为 200；postgres/server/web/nginx 正常。Playwright 真实浏览器核对 `/instruction`、A/B 预览：20 元、线上规则、A绩效、B“证据来源”和团队绩效新文案均已显示。开场页验收使用虚拟 Session 仅为绕过页面入口，产生的两条 404 控制台信息来自虚拟 runtime 请求，未写数据库。
- 上传题库的主机/容器 `/tmp` 中转文件已删除；未跟踪的 `.obsidian/`、`.playwright-cli/`、部署图片和本地证书目录未纳入 Git、未清理。

---

### 2026-09-06 正式实验文本收尾、项目文档统一与全流程终审

**参与者文本与 Word**：
- `/instruction` 第五节标题后补充“为保证线上实验按照统一方式进行，请在进入后续实验前阅读以下规则。”；A 第一页材料规则统一为“如果该材料完全不包含相关线索，则选择‘未发现’，无需填写证据。”。
- A 任务表网页和会议 Word 的团队绩效统一为“团队绩效计算A和B均经手处理并有提交记录的公司。”。
- V3.0 问卷 Word 的内部退出说明收口为“正式工作阶段若队友退出，仍在线成员按当前线上实施方案继续实验。”，没有向被试额外解释 A/B 退出分支。
- 本地三份 Word 已通过 Microsoft Word PDF 导出和逐页图片检查，共 17 页，无裁切、重叠或乱码。`00_start_materials/` 按项目规则继续被 git 忽略，网页正式文本与规格文档已提交。

**线上质量与导出补强**：
- `company_metadata.json.timing` 显式新增 `bCanSubmitAt`，直接导出数据库原值；清洗不再必须按 `bAssignedAt + 300 秒` 猜测。
- 复制/粘贴事件显式关联工作段、任务和公司；服务端验证任务属于当前 Session 后用数据库回填公司，避免客户端伪造或错误归属。
- 当前标签页用 `sessionStorage` 保留最近 50 个加盐复制哈希，页面刷新或连续复制多段内容后仍能识别平台内粘贴；仍不保存剪贴板原文。
- 掉线开始时按掉线起点关闭已有 OFFSCREEN 区间，再开 DISCONNECT，避免两种时长重复累计；新增对应单元测试。
- `online_integrity.json.events[]` 现在显式带 `eventId/role/segmentIndex/taskAssignmentId/companyId`，可以按个人、工作段和公司聚合。
- 生产环境 `/dev/*` 开发预览路由统一返回 404，避免误用预览页或向真实 Session 写测试进度。

**文档系统维护**：
- 系统性统一根 README、英文 README、技术栈、项目规则、`启动prompt.txt`、前后端 README、前端/后端/变量/问卷/部署规格，清理 A0-A6、A0 单独无 AI、V2.2/V0.2、截图未实现等过期当前口径；历史时间线内容保留并明确标注历史。
- `02_specs/README.md` 建立当前重要文档导航；服务器文档明确 HTTPS 方案、通用 Nginx 厂商文档边界、新证书序列号与回滚路径。
- 新增 `02_specs/03_execution/正式实验前全流程终审_20260906.md`，覆盖完整流程、线上极端场景、数据库/导出字段映射、已知边界、15 项双人手工验收和放行标准。
- 终审核对第九次会议 V2.0：其七条件、A/B 退出、提前结束短问卷和“不做全屏”均已被后续决定取代；最少事件、切屏/掉线互斥、剪贴板与分析层级思路继续保留。

**终审结论与剩余风险**：
- 主流程、九条件、A/B 异步派单、三段段前指导语、V3.0 问卷、任务2 5×4 暴露、线上质量区间和结构化导出已经闭环，可进入正式双人测试。
- 当前导出具备生成个人/团队 × 总体/工作段/公司的原始和派生数据，但尚未直接物化旧 V2.0 指定的六张 CSV；若师兄把六张表列为硬交付，应先按 A0-A8 最终字段冻结后再开发。
- 参与者 API 仍以 Session/参与者标识识别，没有签名身份 token；正式公网招募前应决定是否将其列为 P0。正式工作前单人离场也仍需 Admin 人工处置。
- 第二屏、手机、虚拟机或失焦后具体打开的软件属于普通网页不可观测边界，只能由规则承诺和最终自报补充。

**验证、GitHub 与生产**：
- Server 8 个测试套件/39 项测试通过；server build、web production build（24 路由）和 `git diff --check` 通过。全库 ESLint 仍有既存格式/类型债务，未在本轮做无关大规模格式化。
- 本地 Playwright 逐字核对 A 指导语和 A 任务表；线上 Playwright 再次确认“未发现”和第五节新增句均存在，生产 `/dev/preview/a` 返回 404。
- 主实现提交 `59c1f1a 统一正式实验文档并补强线上质量导出` 已推送 GitHub `main`，并以 git archive 全量重建生产 web/server；postgres/server/web/nginx 正常，公网 `/api/health`、`/login`、`/admin` 为 200，HTTP→HTTPS 为 301。
- 部署前备份位于 `/opt/multi-cooperation-backups/20260906T214200_final_audit/`：`database.sql` 约 1.4 MB、`server_storage.tar.gz` 约 106 MB，均已生成 SHA-256；没有删除或重建生产 volume。
