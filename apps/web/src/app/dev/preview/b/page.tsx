...
async function recordStage(code: string, role: "A" | "B", stage: string) {
  try {
    await fetch(`${serverBaseUrl}/experiment/session/${code}/progress`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        role,
        stage,
        payload: { source: "web", page: "preview-b" },
      }),
      cache: "no-store",
    });
  } catch {}
...
export default async function PreviewBPage(props: {
          ...
          content: (
            <div className="space-y-6">
              <div className="rounded-lg border border-[#e5e6eb] bg-white p-5">
                ...
                <div className="mb-8 rounded-r-lg border-l-4 border-[#1e80ff] bg-blue-50 p-4">
                  <p className="mb-2 font-bold text-[#1e80ff]">
                    A 的核心交接备注：
                  </p>
                  <p className="leading-relaxed text-gray-700">
                    这家公司虽然营收增长极快，但海外收入的合规证明还不够扎实，建议重点审查第二季度法务与海外合规文件。
                  </p>
                </div>
                <div className="space-y-6">
                  <div className="rounded-lg border border-[#e5e6eb] bg-gray-50 p-5">
                    <h3 className="mb-3 text-base font-bold text-gray-800">
                      机会点初步梳理
                    </h3>
                    <ul className="list-disc space-y-2 pl-5 text-gray-600">
                      <li>制造业数字化需求真实存在，行业空间不低。</li>
                      <li>若标准化模块跑通，收入质量有改善空间。</li>
                      <li>重点客户场景刚需明确，项目落地价值较强。</li>
                    </ul>
                  </div>
                  <div className="rounded-lg border border-[#e5e6eb] bg-gray-50 p-5">
                    <h3 className="mb-3 text-base font-bold text-gray-800">
                    ...
                    <ul className="list-disc space-y-2 pl-5 text-gray-600">
                      <li>客户集中与交付依赖仍是压顶问题。</li>
                      <li>毛利率波动较大，产品化能力仍需验证。</li>
                    </ul>
                  </div>
                </div>
              </div>
        ...
        },
        {
          ...
          content: (
            <div className="space-y-4">
              {mockCompany.rawMaterials.map((material) => (
                <div
                  key={material.title}
                  className="rounded-lg border border-[#e5e6eb] bg-gray-50 p-5"
                >
          ...
          <WorkbenchLayout
            ...
            taskPane={
              <div className="no-scrollbar h-full overflow-y-auto p-5">
                <BTaskEditor sessionCode={code} initialData={mockBWorkspace} />
              </div>
            }
            aiPane={
              <AiChatPanel sessionCode={code} role={role} accent="purple" />
            }
            taskTitle="我的判定任务"
            aiTitle="AI 分析助手"
          />
