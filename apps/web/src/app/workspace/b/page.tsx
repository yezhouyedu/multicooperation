...
export default function WorkspaceBPage() {
  ...
  const [viewingDiligenceInfo, setViewingDiligenceInfo] = useState(false);
  ...
  const diligenceTabContent = !runtime?.aInfoUnlocked ? (
    <div className="flex min-h-[260px] flex-col items-center justify-center rounded-xl border border-dashed border-[#c9cdd4] bg-gray-50 p-6 text-center text-sm text-[#86909c]">
      ...
      <div>你可以先阅读自有材料、填写投资判断并使用 AI。尽调员提交后，这里会自动解锁。</div>
    </div>
  ) : !runtime.bHasViewedAInfo && !viewingDiligenceInfo ? (
    <div className="rounded-xl border border-blue-200 bg-blue-50 p-5 text-sm leading-7 text-[#1e80ff]">
      <div className="mb-2 text-base font-bold text-[#1d2129]">尽调员信息已到达</div>
      <div>请先打开并查看这部分信息。查看后，提交按钮才会解锁。</div>
      <button
        type="button"
        onClick={() => void openDiligenceInfo()}
        className="mt-4 rounded-lg bg-[#1e80ff] px-4 py-2 text-sm font-bold text-white hover:bg-blue-600"
      >
        打开尽调员信息
      </button>
    </div>
                  ...
                  prependItems={[
                    {
                      ...
                      onSelect: () => void openDiligenceInfo(),
                    },
                  ]}
                />
              }
              ...
              taskPane={
                <div className="flex h-full min-h-0 flex-col">
                  <div className="flex shrink-0 items-center justify-between gap-4 border-b border-[#e5e6eb] px-5 py-3 text-xs text-[#86909c]">
                    <span>你可以先形成初步判断；只有在尽调员信息解锁并查看后，才能进入反馈与提交。</span>
                    <button
                      type="button"
