...
function InstructionSection({
  ...
  return (
    <section className="border-b border-[#f0f2f5] py-8 last:border-b-0">
      <h2 className="mb-4 text-[16px] font-semibold text-[#1d2129]">{title}</h2>
      {children}
    </section>
  );
}
...
function ParagraphList({ items }: { items: string[] }) {
  return (
    <div className="space-y-3">
      {items.map((item) => (
        <p key={item} className="text-[15px] leading-[2] text-[#4e5969]">
          {item}
        </p>
      ))}
    </div>
...
function RoleParagraphList({ items }: { items: string[] }) {
  return (
    <div className="space-y-3">
      {items.map((item, index) => {
        // 第一条是"你在本实验中的角色是X"，单独高亮
        if (index === 0) {
          return (
            <p key={item} className="text-[15px] font-medium leading-[2] text-[#1d2129]">
              {item}
            </p>
          );
        }
        ...
        if (/^\d+\./.test(item)) {
          return (
            <p key={item} className="pl-4 text-[15px] leading-[2] text-[#4e5969]">
              {item}
            </p>
          );
        }
        ...
        <div className="w-full max-w-4xl">
          ...
          >
            ...
            <header className="border-b border-[#eaecf0] px-10 py-8">
              ...
              {role ? (
                <div className="mt-4 inline-block rounded-md border border-[#e5e6eb] bg-[#f7f8fa] px-4 py-2 text-[13px] text-[#4e5969]">
                  当前角色：{role === 'A' ? '角色A' : '角色B'}
                </div>
              ) : null}
            </header>
            ...
            <div className="px-10">
              {/* 实验流程（如果后端有配置） */}
              {experimentFlow.trim() ? (
                <InstructionSection title="一、实验流程">
                  <div className="space-y-3">
                    {experimentFlow
                      .split(/\r?\n|；|;/)
                      ...
                      .filter(Boolean)
                      .map((item, index) => (
                        <p key={`${item}-${index}`} className="text-[15px] leading-[2] text-[#4e5969]">
                          {index + 1}. {item}
                        </p>
                      ))}
                  </div>
              ...
              {/* 任务介绍（来自通用指导语.docx） */}
              <InstructionSection title="二、任务介绍">
                <ParagraphList items={commonInstructions} />
              </InstructionSection>

              {/* 机会/风险判断说明（来自通用指导语.docx） */}
              <InstructionSection title="三、机会/风险判断说明">
                <ParagraphList items={opportunityRiskInstructions} />
              </InstructionSection>

              {/* 角色说明（来自角色A/B指导语.docx） */}
              <InstructionSection title="四、角色说明">
                <RoleParagraphList items={roleSpecific} />
              </InstructionSection>

              {/* 实验条件额外提示（如有） */}
              {activeModeText.trim() ? (
                <section className="py-8">
                  <div className="rounded-lg border border-[#e5e6eb] bg-[#f7f8fa] px-5 py-4 text-[14px] leading-7 text-[#4e5969]">
                    {activeModeText}
                  </div>
                </section>
              ) : null}
