  ...
  return (
    <main className="flex h-screen flex-col bg-[#f0f2f5] text-[#1d2129]">
      ...
      <div className="no-scrollbar flex-1 overflow-y-auto p-6">
        <div className="mx-auto max-w-3xl rounded-xl border border-[#e5e6eb] bg-white p-8 shadow-sm">
          <div className="mb-2 text-xs font-bold tracking-widest text-[#86909c]">休息阶段</div>
          <div className="mb-4 text-2xl font-bold text-[#1d2129]">{questionnaire?.title ?? '休息问卷'}</div>
          <div className="mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-[#1e80ff]">
            当前休息剩余时间：{countdownLabel}
          </div>

          {submitted ? (
            <div className="rounded-xl border border-green-200 bg-green-50 p-6 text-sm leading-7 text-green-700">
              <div className="mb-1 text-base font-bold">问卷已提交</div>
            ...
            <>
              <div className="space-y-6">
                {(questionnaire?.items ?? []).map((item, index) => (
                  <section key={item.id} className="rounded-lg border border-[#e5e6eb] p-5">
                    <div className="mb-3 font-bold text-[#1d2129]">
